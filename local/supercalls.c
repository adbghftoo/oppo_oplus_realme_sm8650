...
int ksu_handle_sys_reboot(int magic1, int magic2, unsigned int cmd, void __user **arg)
{
	...
	if (magic2 == SUSFS_MAGIC && current_uid().val == 0) {
		...
		if (cmd == CMD_SUSFS_ADD_SUS_PATH_LOOP) {
			susfs_add_sus_path_loop((struct st_susfs_sus_path __user *)(*arg));
			return 0;
		}
		if (cmd == CMD_SUSFS_SET_ANDROID_DATA_ROOT_PATH) {
			susfs_set_i_state_on_external_dir((struct st_susfs_sus_path __user *)(*arg));
			return 0;
		}
		...
		if (cmd == CMD_SUSFS_HIDE_SUS_MNTS_FOR_ALL_PROCS) {
			susfs_set_hide_sus_mnts_for_all_procs((struct st_susfs_sus_mnt_id __user *)(*arg));
			return 0;
		}
		...
		if (cmd == CMD_SUSFS_SHOW_SUS_SU_WORKING_MODE) {
			susfs_get_sus_su_working_mode((int __user *)(*arg));
			return 0;
		}
		if (cmd == CMD_SUSFS_IS_SUS_SU_READY) {
			susfs_is_sus_su_ready((bool __user *)(*arg));
			return 0;
		}
		if (cmd == CMD_SUSFS_SUS_SU) {
			susfs_sus_su((struct st_susfs_sus_su_input __user *)(*arg));
			return 0;
		}
		...
		if (cmd == CMD_SUSFS_SHOW_ENABLED_FEATURES) {
			susfs_get_enabled_features((u64 __user *)(*arg));
			return 0;
		}
		if (cmd == CMD_SUSFS_SHOW_VARIANT) {
			susfs_show_variant((char __user *)(*arg));
			return 0;
		}
		if (cmd == CMD_SUSFS_SHOW_VERSION) {
			susfs_show_version((char __user *)(*arg));
			return 0;
		}
		return 0;
	}
